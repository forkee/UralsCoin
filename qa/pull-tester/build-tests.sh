...
if [ -d "$OUT_DIR" -a -w "$OUT_DIR" ]; then
  set +e
  # Windows:
  cp /home/wizard/UralsCoin/win32-build/src/bitcoind.exe $OUT_DIR/bitcoind.exe
  cp /home/wizard/UralsCoin/win32-build/src/test/test_urals.exe $OUT_DIR/test_urals.exe
  cp /home/wizard/UralsCoin/win32-build/src/qt/bitcoind-qt.exe $OUT_DIR/bitcoin-qt.exe
  # Linux:
  cp /home/wizard/UralsCoin/linux-build/src/bitcoind $OUT_DIR/bitcoind
  cp /home/wizard/UralsCoin/linux-build/src/test/test_urals $OUT_DIR/test_urals
  cp /home/wizard/UralsCoin/linux-build/src/qt/bitcoind-qt $OUT_DIR/bitcoin-qt
  set -e
fi

# Run unit tests and blockchain-tester on Linux:
cd /home/wizard/UralsCoin/linux-build
make check

# Run RPC integration test on Linux:
/home/wizard/UralsCoin/qa/rpc-tests/wallet.sh /home/wizard/UralsCoin/linux-build/src
/home/wizard/UralsCoin/qa/rpc-tests/listtransactions.py --srcdir /home/wizard/UralsCoin/linux-build/src
# Clean up cache/ directory that the python regression tests create
rm -rf cache

if [ $RUN_EXPENSIVE_TESTS = 1 ]; then
  # Run unit tests and blockchain-tester on Windows:
  cd /home/wizard/UralsCoin/win32-build
  make check
fi
