...
void ParseParameters(int argc, const char* const argv[])
{
    ...
    BOOST_FOREACH(const PAIRTYPE(string,string)& entry, mapArgs)
    {
        ...
        if (name.find("--") == 0)
        {
            std::string singleUrals(name.begin()+1, name.end());
            if (mapArgs.count(singleUrals) == 0)
                mapArgs[singleUrals] = entry.second;
            name = singleUrals;
        }
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "urals.conf"));
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "uralsd.pid"));
    ...
    if (vTimeOffsets.size() >= 5 && vTimeOffsets.size() % 2 == 1)
    {
        ...
        else
        {
            ...
            if (!fDone)
            {
                ...
                {
                    fDone = true;
                    string strMessage = _("Warning: Please check that your computer's date and time are correct! If your clock is wrong Urals will not work properly.");
