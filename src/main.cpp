...
#if defined(NDEBUG)
# error "Urals cannot be compiled without assertions."
#endif
...
const string strMessageMagic = "Urals Signed Message:\n";
...
{
    	int64_t ret = blockValue / 5; // start at 20% URALS Target is in future 50% by over 2000 Masternodes
...
static const int64_t nTargetTimespan = 6 * 24 * 60 * 60; // URALS 5 Day
static const int64_t nTargetSpacing = 5 * 60; // Urals: 5 minutes   URALS
...
unsigned int GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
	unsigned int retarget = DIFF_NULL;
	 ...
	 // Urals: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CheckBlock(const CBlock& block, CValidationState& state, bool fCheckPOW, bool fCheckMerkleRoot)
{
    ...
    // Uralsdev: limit timestamp window 
   ...
   // ----------- masternode payments -----------
    ...
    if(!IsSporkActive(SPORK_1_MASTERNODE_PAYMENTS_ENFORCEMENT)){
        MasternodePayments = true; // Uralsdev
    ...
    if(MasternodePayments)
    {
        ...
        if(pindex != NULL){
            if(pindex->GetBlockHash() == block.hashPrevBlock){
				...
				int64_t hardblockpowreward = block.vtx[0].vout[0].nValue; //write by Uralsdev 02-06-2015
				...
				//Uralsdev 18-10-2015 Urals proof of payment Number 2
...
bool AcceptBlock(CBlock& block, CValidationState& state, CDiskBlockPos* dbp)
{
    ...
    if (hash != Params().HashGenesisBlock()) {
        ...
        }*/ //uralsdev Prüfunktion 25-05-20105
