...
Value darksend(const Array& params, bool fHelp)
{
    if (fHelp || params.size() == 0)
        throw runtime_error(
            "darksend <uralsaddress> <amount>\n"
            "uralsaddress, reset, or auto (AutoDenominate)"
    ...
    if (params.size() != 2)
        throw runtime_error(
            "darksend <uralsaddress> <amount>\n"
            "uralsaddress, denominate, or auto (AutoDenominate)"
            "<amount> is a real and is rounded to the nearest 0.00000001"
            + HelpRequiringPassphrase());

    CUralsAddress address(params[0].get_str());
    if (!address.IsValid())
...
Value masternode(const Array& params, bool fHelp)
{
    ...
    if (fHelp  ||
        ...
        throw runtime_error(
                ...
                "  vote-many    - Vote on a Urals initiative\n"
                "  vote         - Vote on a Urals initiative\n"
                );
