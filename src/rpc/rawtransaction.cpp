...
UniValue createrawtransaction(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() < 2 || request.params.size() > 3)
        throw runtime_error(
            ...
            "    {\n"
            "      \"address\": x.xxx,    (numeric or string, required) The key is the urals address, the numeric value (can be string) is the " + CURRENCY_UNIT + " amount\n"
    ...
    BOOST_FOREACH(const string& name_, addrList) {
        ...
        if (name_ == "data") {
            ...
            CUralsAddress address(name_);
            if (!address.IsValid())
                throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, string("Invalid urals address: ")+name_);
