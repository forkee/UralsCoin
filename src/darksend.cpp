...
/* *** BEGIN DARKSEND MAGIC - URALS **********
    Copyright (c) 2014-2015, Urals Developers
        eduffield - evan@uralspay.io
    ...
    if(state == POOL_STATUS_FINALIZE_TRANSACTION) {
      ...
      for(unsigned int i = 0; i < entries.size(); i++){
            ...
            txNew.vout.push_back(v);
                ...
                std::random_shuffle ( txNew.vout.begin(), txNew.vout.end(), randomizeList); //uralsdev
    ...
    if(fMasterNode) {
        ...
        BOOST_FOREACH(const CTransaction& txCollateral, vecSessionCollateral) {
            ...
            /*
                ...
                allow endless transaction that would bloat Urals and make it unusable. To
...
bool CDarksendPool::DoAutomaticDenominating(bool fDryRun, bool ready)
{
 ...
 if(!sessionFoundMasternode){
        ...
        LogPrintf("Submitting Darksend for %f URALS CENT - sessionTotalValue %d\n", fDarkcoinSubmitted, sessionTotalValue);
...
LogPrintf("DoAutomaticDenominating -- Invalid collateral, resetting.\n");
        ...
        if(nUseQueue > 33){
            ...
            BOOST_FOREACH(CDarksendQueue& dsq, vecDarksendQueue){
                ...
                BOOST_FOREACH(CTxIn usedVin, vecMasternodesUsed){
                    if(dsq.vin == usedVin) {
                        fUsed = true; // UralsDev 08.05.2015 master (#559)
                        break;
                    }
                }
				if(fUsed) continue;
                ...
                if(ConnectNode((CAddress)addr, NULL, true)){
                    ...
                    dsq.time = 0; //remove node
                    continue; // UralsDev 08.05.2015 master (#559)
                }
            }
        }
        ...
        int i = 0;
		// UralsDev 08-05-2016 master (#623) 
...
{
    std::string strError = "";
	//UralsDev 08-05-2016      master (#607) 
