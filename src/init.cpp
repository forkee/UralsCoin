...
std::string HelpMessage(HelpMessageMode hmm)
{
    ...
    strUsage += "  -conf=<file>           " + _("Specify configuration file (default: urals.conf)") + "\n";
    if (hmm == HMM_BITCOIND)
    {
#if !defined(WIN32)
    ...
    strUsage += "  -pid=<file>            " + _("Specify pid file (default: uralsd.pid)") + "\n";
    ...
    strUsage += "  -port=<port>           " + _("Listen for connections on <port> (default: 7444 or testnet: 17444)") + "\n";
    ...
    strUsage += "  -anonymizeuralsamount=<n> " + _("Keep N urals anonymized (default: 0)") + "\n";
    ...
    strUsage += "  -rpcport=<port>        " + _("Listen for JSON-RPC connections on <port> (default: 7443 or testnet: 17443)") + "\n";
...
/** Sanity checks
 *  Ensure that Urals is running in a usable environment with all
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. Urals Core is shutting down."));
...
#endif
    // Make sure only a single Urals process is using the data directory.
    boost::filesystem::path pathLockFile = GetDataDir() / ".lock";
    FILE* file = fopen(pathLockFile.string().c_str(), "a"); // empty lock file; created if it doesn't exist.
    if (file) fclose(file);
    static boost::interprocess::file_lock lock(pathLockFile.string().c_str());
    if (!lock.try_lock())
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. Urals Core is probably already running."), strDataDir));
...
if(nWalletBackups > 0)
    ...
    if (fDisableWallet) {
        ...
        if (nLoadWalletRet != DB_LOAD_OK)
        {
            ...
            else if (nLoadWalletRet == DB_NONCRITICAL_ERROR)
            {
                ...
                strErrors << _("Error loading wallet.dat: Wallet requires newer version of Urals") << "\n";
            else if (nLoadWalletRet == DB_NEED_REWRITE)
            {
                strErrors << _("Wallet needed to be rewritten: restart Urals to complete") << "\n";
    ...
    fProUserModeDarksendInstantX = GetBoolArg("-promode", false); //Uralsdev im Standart an (Darksend und Instantx ist im QT nicht sichtbar)
	fProUserModeDarksendInstantX2 = GetBoolArg("-disable_DS_InstantX", false);  // Uralsdev im Standart aus (Darksend und Instantx ist im Core an)
    ...
    LogPrintf("Anonymize Urals Amount %d\n", nAnonymizeDarkcoinAmount);
    ...
    nAnonymizeDarkcoinAmount = GetArg("-anonymizeuralsamount", 0);
    ...
    */
    darkSendDenominations.push_back( (100      * COIN)+100000 );  //Uralsdev for a quick sync 08-05-2016
