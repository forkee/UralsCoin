 ...
 * between uralsd, and urals-qt and non-server tools.
*/
...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    strUsage += HelpMessageOpt("-conf=<file>", strprintf(_("Specify configuration file (default: %s)"), URALS_CONF_FILENAME));
    if (mode == HMM_URALSD)
    {
#if HAVE_DECL_DAEMON
...
#ifndef WIN32
    strUsage += HelpMessageOpt("-pid=<file>", strprintf(_("Specify pid file (default: %s)"), URALS_PID_FILENAME));
#endif
...
{
    const std::string URL_SOURCE_CODE = "<https://github.com/JohnMnemonick/UralsCoin>";
    const std::string URL_WEBSITE = "<https://uralscoin.info>";

	//TODO-- add urals license
    return FormatParagraph(strprintf(_("Copyright (C) 2009 -%i The Bitcoin Core developers"), COPYRIGHT_YEAR)) + "\n" +
		   FormatParagraph(strprintf(_("Copyright (C) 2014 -%i The Dash Developers"), COPYRIGHT_YEAR)) + "\n" +
           FormatParagraph(strprintf(_("Copyright (C) 2015 -%i The Urals Core Developers"), COPYRIGHT_YEAR)) + "\n" +
           FormatParagraph(strprintf(_("Copyright (C) 2017 -%i The Urals Core Developers"), COPYRIGHT_YEAR)) + "\n" + "\n" +
    ...
    try {
       ...
       // Wait maximum 10 seconds if an old wallet is still running. Avoids lockup during restart
        if (!lock.timed_lock(boost::get_system_time() + boost::posix_time::seconds(10)))
            return InitError(strprintf(_("Cannot obtain a lock on data directory %s. Urals is probably already running."), strDataDir));
        if (probeOnly) {
            lock.unlock();
        }
...
bool AppInitMain(boost::thread_group& threadGroup, CScheduler& scheduler)
{
    ...
    LogPrintf("Using config file %s\n", GetConfigFile(GetArg("-conf", URALS_CONF_FILENAME)).string());
	...
	if (mapMultiArgs.count("-masternodepaymentskey")) // masternode payments priv key
    {
        if (!masternodePayments.SetPrivKey(GetArg("-masternodepaymentskey", ""))){
			...
			//LogPrintf("Masternode: Create new key %s", CUralsSecret(secret).ToString());
