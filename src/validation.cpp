...
const std::string strMessageMagic = "Urals Signed Message:\n";
                              ...
                              bool fOverrideMempoolLimit, const CAmount& nAbsurdFee, std::vector<uint256>& vHashTxnToUncache, bool fDryRun)//TODO--
{
...

    {
        ...
        {
            LogPrint("mempool", "replacing tx %s with %s for %s BSD additional fees, %d delta bytes\n",
    ...
    if(MasternodePayments)
    {
        ...
        if(pindex != NULL){
            if(pindex->GetBlockHash() == block.hashPrevBlock){
				...
				CAmount hardblockpowreward = block.vtx[0]->vout[0].nValue; //write by Uralsdev 02-06-2015//todo++
